...
class GameCharacterTest extends TestCase
{
    ...
    public function test_model_uses_correct_fillable_attributes(): void
    {
        ...
        $this->assertContains('vitality', $fillable);
        $this->assertContains('energy', $fillable);
    }
    ...
    public function test_model_uses_correct_casts(): void
    {
        ...
        $this->assertArrayHasKey('hp_potion_threshold', $casts);
        $this->assertArrayHasKey('mp_potion_threshold', $casts);
        $this->assertArrayHasKey('combat_skills_used', $casts);
        $this->assertArrayHasKey('combat_skill_cooldowns', $casts);
        $this->assertArrayHasKey('combat_monsters', $casts);
    }
    ...
    public function test_clear_combat_state_resets_all_combat_fields(): void
    {
        $character = new GameCharacter([
            'combat_monster_id' => 1,
            'combat_monster_hp' => 100,
            'combat_monster_max_hp' => 200,
            'combat_monsters' => [['hp' => 100, 'id' => 1]],
            'combat_total_damage_dealt' => 500,
            'combat_total_damage_taken' => 200,
            'combat_rounds' => 10,
        ]);
        ...
        $this->assertNull($character->combat_monster_max_hp);
        $this->assertNull($character->combat_monsters);
        $this->assertEquals(0, $character->combat_total_damage_dealt);
        $this->assertEquals(0, $character->combat_total_damage_taken);
        $this->assertEquals(0, $character->combat_rounds);
    }
    ...
    public function test_current_map_relationship_exists(): void
    {
        $this->assertTrue(method_exists($this->character, 'currentMap'));
    }

    public function test_combat_logs_relationship_exists(): void
    {
        $this->assertTrue(method_exists($this->character, 'combatLogs'));
    }
}
